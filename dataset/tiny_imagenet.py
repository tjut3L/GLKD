...
def get_data_folder():
    ...
    """
    data_folder = '/data/tjut_jjl/dataset/tiny-imagenet-200'
...
def get_dataloader_sample(dataset='tiny-imagenet', batch_size=128, num_workers=8, is_sample=False, k=4096):
    ...
    train_set = datasets.ImageFolder(train_folder, transform=test_transform)
    test_set = datasets.ImageFolder(test_folder, transform=test_transform)
...
def get_tiny_imagenet_dataloader(dataset='tiny-imagenet', batch_size=128, num_workers=16, is_instance=False):
    """
    Data Loader for imagenet
    """
    if dataset == 'tiny-imagenet':
    ...
    if is_instance:
        train_set = ImageFolderInstance(train_folder, transform=MultiCropAugmentation((0.4, 0.7), 1))
        n_data = len(train_set)
    else:
        train_set = datasets.ImageFolder(train_folder, transform=MultiCropAugmentation((0.4, 0.7), 1))
...
class MultiCropAugmentation(object):
    def __init__(self, local_crops_scale, local_crops_number):
        ...
        self.local_transform = transforms.Compose([
            transforms.Resize(72),
            transforms.RandomResizedCrop(64, scale=local_crops_scale, interpolation=Image.BICUBIC),
            transforms.RandomHorizontalFlip(),
            transforms.ToTensor(),
            normalize,
        ])
