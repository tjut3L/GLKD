...
from models.util import Connector, Translator, Paraphraser, SRRL
...
def parse_option():
    ...
    parser.add_argument('--method', type=str, default='dkd')
    parser.add_argument('--scratch', type=str, default='0')
    ...
    if hostname.startswith('visiongpu'):
        ...
        if opt.scratch == '1':
            opt.model_path = './save/student_models_scratch/{}/{}'.format(opt.distill, opt.method)
            opt.tb_path = './save/student_tensorboard_scratch/{}/{}'.format(opt.distill, opt.method)
        else:
            opt.model_path = './save/student_models/{}/{}'.format(opt.distill, opt.method)
            opt.tb_path = './save/student_tensorboard/{}/{}'.format(opt.distill, opt.method)
    ...
    if opt.distill == 'ctkd':
        if opt.method != '':
            opt.model_name = 'S:{}_T:{}_{}_{}_{}r:{}_a:{}_b:{}_s:{}_t:{}_{}'.format(opt.model_s, opt.model_t,
                                                                                    opt.dataset,
                                                                                    opt.distill, opt.method,
